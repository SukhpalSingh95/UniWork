...
print('')
print('')
#DataSetIn = pd.read_csv('rp1.txt', header=None, index_col=None)#, index=['Profit Of customer', 'Number of requests','Requirements list']) #sep=' ')#,comment='#')  # https://stackoverflow.com/questions/21546739/load-data-from-txt-with-pandas + https://stackoverflow.com/questions/28200404/pandas-read-table-use-first-column-as-index#
...
class MakingSenseOfTheDataFile:
    ...
    def CrowdDistancing(self,populationtoswap): ######## isnt actually used
    ...
    def CrowdDistancing1(self,populationtoswap):    ###### isnt actually used
...
InputPopulation = InputPopulation.sort_values(['Cost Fitness'], ascending=False)                                     ############### NEED TO CHANGE THIS TO SORT FOR A SPECIFIC OBJECTIVE
#InputPopulation = InputPopulation.sort_values(['Score/Fitness'], ascending=False)                                     ############### NEED TO CHANGE THIS TO SORT FOR A SPECIFIC OBJECTIVE

print('')
print(InputPopulation)




print(len(InputPopulation))



CostFitness = []
InputPopulation=InputPopulation.rename(columns={'Cost Fitness':'Old CFitness'})
for i in range(1,len(InputPopulation)):
    normaliser=InputPopulation['Old CFitness'].sum()
    CostFitness.append((InputPopulation.iloc[i][1] / normaliser))
SeriesCostFitness = pd.Series(CostFitness)
SeriesCostFitness = SeriesCostFitness.rename('Cost Fitness', inplace=True)
InputPopulationDF = InputPopulation.reset_index(drop=True)  # https://stackoverflow.com/questions/20490274/how-to-reset-index-in-a-pandas-data-frame
DataframeForDays3 = pd.concat([InputPopulationDF, SeriesCostFitness], axis=1)
InputPopulationDF=DataframeForDays3
InputPopulationDF=InputPopulationDF[['Req Num','Cost Fitness', 'Score/Fitness','Average Fitness']]
print(InputPopulationDF.head())
print('')
...
ScoreFitness = []
for i in range(1,len(InputPopulationDF)):
    normaliser=InputPopulationDF['Score/Fitness'].sum()
    ScoreFitness.append((InputPopulationDF.iloc[i][1] / normaliser))
SeriesCostFitness = pd.Series(ScoreFitness)
SeriesCostFitness = SeriesCostFitness.rename('Score Fitness', inplace=True)
InputPopulationDF2 = InputPopulationDF.reset_index(drop=True)  # https://stackoverflow.com/questions/20490274/how-to-reset-index-in-a-pandas-data-frame
DataframeForDays3 = pd.concat([InputPopulationDF2, SeriesCostFitness], axis=1)
InputPopulationDF2=DataframeForDays3
#print(InputPopulationScorersDF.head())
print('')
InputPopulationDF2=InputPopulationDF2[['Req Num','Cost Fitness','Score Fitness','Average Fitness']]
print(InputPopulationDF2.head())



##########################  FIX UP THE POPULATIONS INTO SEPARATE ONES       THIS ALSO FIXES THE FITNESSES INTO THE SAME ONES AS THE MULTI OBJECTIVE
InputPopulation=InputPopulationDF2
print(len(InputPopulation))


fivepercentpop = int(0.02 * (len(InputPopulation)))
tenpercentpop = int(0.06 * (len(InputPopulation)))
twentypercentpop = int(0.1 * (len(InputPopulation)))
thirtypercentpop = int(0.14 * (len(InputPopulation)))
fourtypercentpop = int(0.18 * (len(InputPopulation)))
...
NumOfIterations=NumberOfGenerations
populations2=[]
itterationnumber=0
...
plt.legend(loc='best')
#plt.title('Multi Objective - Customer happiness orientated- Classic-nrp',fontsize=16)
#plt.title('Single Objective - Cost Fitness dominated - Realistic:nrp-m4',fontsize=16)
#plt.title('Single objective - Cost fitness orientated - Trial 2 - Classic-nrp',fontsize=16)
plt.title('Single objective - Cost fitness orientated - Trial 1 - Realistic-nrp-m4',fontsize=16)
...
#plt.savefig('Single Objective - big - Cost 1.pdf')
#plt.savefig('Single Objective - score dominant - small 2.pdf')
